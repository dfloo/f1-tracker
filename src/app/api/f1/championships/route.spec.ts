...
describe('GET /api/f1/championships', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.stubEnv('API_SERVER_URL', 'https://backend.example.com');
  });

  it('proxies upstream response for valid year', async () => {
    ...
    const request = new NextRequest(
      'http://localhost:3000/api/f1/championships?year=2024',
    );
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://backend.example.com/api/f1/championships?year=2024',
  ...
  it('returns 400 for invalid year values', async () => {
    ...
    const request = new NextRequest(
      'http://localhost:3000/api/f1/championships?year=not-a-year',
    );
  ...
  it('returns 400 for empty year values', async () => {
    ...
    const request = new NextRequest(
      'http://localhost:3000/api/f1/championships?year=',
    );
